...
/**
 * NRD · updater-ui.js — premium in-app auto-update experience.
 * Banner states: available → downloading (live %, MB, ETA) → downloaded (restart).
 */
(function () {
  let banner = null;
  let lastState = null;
  ...
  function render(state) {
    ...
    switch (state.status) {
        ...
        setActions([
          {
            label: 'Update Now',
            cls: 'btn-gold',
            onClick: () => window.nrd.updaterDownload(),
          },
      ...
      case 'downloading': {
        ...
        b.querySelector('.ub-mb').textContent =
          `${state.transferredMb} MB / ${state.totalMb} MB`;
        b.querySelector('.ub-eta').textContent =
          `${fmtBytesPerSec(state.bytesPerSecond)} · ${fmtEta(state.etaSec)}`;
        setActions([]);
        break;
      }
        ...
        setActions([
          {
            ...
            cls: 'btn-gold',
            onClick: () => window.nrd.updaterInstall(),
          },
          { label: 'Later', cls: 'btn-ghost', onClick: hideBanner },
        ]);
        window.NRDToast.show({
      ...
      case 'error':
        hideBanner();
        // Silent on background checks; Settings page surfaces details.
  ...
  window.NRDUpdaterUI = {
    ...
    /** Manual refresh after Settings "Check for Updates" press */
    render,
